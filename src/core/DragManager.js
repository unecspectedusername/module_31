...
class DragManager {
  constructor() {
    ...
    this.oldColumn = null;
    // колонка kanban в которую мы помещаем задачу
    this.newColumn = null;
  ...
  doWhileMoving(event) {
    ...
    // сохраняем колонку под курсором, как целевую на будущее
    this.newColumn = columnBelow;
  ...
  finish() {
    ...
    if (this.placeholder && this.draggedTask) {
      // находим индекс, по которому будет установлена новая задача
      const index = Array.from(this.placeholder.parentNode.childNodes).indexOf(this.placeholder);
